<?php
	...
	if ( !isset($_COOKIE['user']) )
		setcookie("user",md5("nobody"), time()+3600);
	setcookie("ipsession",md5($_SERVER['REMOTE_ADDR']),time()+3600);
	setcookie("ticket",md5(time()-0xdeaddead),time()+600);
?>
...
<body style="padding: 0px; margin: 0px; background: #ffffff; font-family: verdana,times; font-size: small">
		<table style="border-color: #000000; border-style:solid; border-width:1px; width:750px; background-image: url('gfx/tlo.jpg')" cellspacing="0" cellpadding="0" align="center">
			...
			<tr>
				...
				<a href="index.py?dzial=ofirmie">O firmie</a> | <a href="index.py?dzial=oferta">Oferta</a> | <a href="index.py?dzial=praca">Praca</a> | <a href="index.py?dzial=kontakt">Kontakt</a></big>
				</td>
			</tr>
<?php
	...
	{
		case "praca":
			include('Jushoom3/praca.py');
			break;
		case "oferta":
			include('Jushoom3/oferta.py');
			break;
		case "kontakt":
			include('Jushoom3/kontakt.py');
			break;
		case "admin":
			include('Jushoom3/admin.py');
			break;
		default:
			include('Jushoom3/ofirmie.py');
			break;
	}
